...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image, image2, label, label2,mask0_1):
        img1, img2, mask1, mask2, mask_bin = Image.fromarray(np.asarray(image)), Image.fromarray(np.asarray(image2)), \
                    Image.fromarray(np.asarray(label)), Image.fromarray(np.asarray(label2)), Image.fromarray(np.asarray(mask0_1))
        ...
        results = [img1, img2, mask1, mask2, mask_bin]
...
class ToTensor(object):
    ...
    def __call__(self, *data):
        ...
        if label is None:
            ...
            return img, img2, torch.LongTensor(np.array(
                label, dtype=np.int)), torch.LongTensor(np.array(label2, dtype=np.int)),torch.LongTensor(np.array(mask0_1, dtype=np.int))
