...
class SPA_attn(nn.Module):
    ...
    def forward(self, x):
        spatial_attn = self.spatial_pool(x)
        return spatial_attn


class CHA_attn(nn.Module):
    def __init__(self):
        super().__init__()
        self.channel_avgpool = nn.AdaptiveAvgPool2d((1, 1))
    ...
    def forward(self, x):
        channel_avg_pool = self.channel_avgpool(x).squeeze()
        channel_attn = self.channel_activation(self.channel_pool(channel_avg_pool)).unsqueeze(-1).unsqueeze(-1)
...
class CA(nn.Module):
    def __init__(self, n_classes):
        super().__init__()
        self.spatial_attn = SPA_attn()

        self.channel_attn1 = CHA_attn()
        self.channel_attn2 = CHA_attn()

        self.conv_color1 = nn.Sequential(nn.Conv2d(1280, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(256),
                                       nn.ReLU(inplace=True),
        ...
        self.conv_color2 = nn.Sequential(nn.Conv2d(1280, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(256),
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(256),
        ...
        self.conv_gray_mid = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(256),
                                       nn.ReLU(inplace=True),
                                       nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(256),
        ...
        self.conv_gray_final = nn.Conv2d(256, 2, kernel_size=1, stride=1)

    def forward(self, x1, x2, input):
        x_gray = torch.abs(x1 - x2)                                     # channel = 512
        spatial_attn = self.spatial_attn(x_gray)
        x_gray = x_gray * spatial_attn

        # binary output
        x_gray_mid = self.conv_gray_mid(x_gray)
        x_gray_final = self.conv_gray_final(x_gray_mid)
        x_gray = F.interpolate(x_gray_final, size=input.size()[2:], mode='bilinear', align_corners=False)

        # semantic output
        x1 = self.channel_attn1(x1)
        x2 = self.channel_attn2(x2)
        
        x_color = torch.cat((x1, x2, x_gray_mid), dim=1)                 # channel = 1280        
        x_color1 = self.conv_color1(x_color)
        x_color1 = F.interpolate(x_color1, size=input.size()[2:], mode='bilinear', align_corners=False)

        x_color2 = self.conv_color2(x_color)
        x_color2 = F.interpolate(x_color2, size=input.size()[2:], mode='bilinear', align_corners=False)

        return x_color1, x_color2, x_gray


class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=6, os=16, pretrained=True, _print=False):
        if _print:
            print("Constructing SSESN_scd model...")
    ...
    def forward(self, input):#input 1, 6, 512, 512
        ...
        # change aware module for SCD
        x_color1, x_color2, x_gray = self.ca(x1, x2, input)
